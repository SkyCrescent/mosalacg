...
import Pied2 from '../../Pied2'
import { usePathname, useRouter  } from "next/navigation";
//import  d from '@/app/home/tare/seeEvents/concerned'
...
let couleur2 = ""
   ...
   const getData2 = async () => {
      try {
         // Remplacez l'URL par la bonne URL de votre API
         const response = await axios.get(`${apiUrl}/infos/get_byId.php?id_evenement=${id}`);
         // console.log(response.data && response.data.recu && response.data.recu.length > 0)
         if (response.data && response.data.recu && response.data.recu.length > 0) {
            // Vérifiez que la réponse contient les données attendues
   ...
   const getData = async () => {
      try {
         // Remplacez l'URL par la bonne URL de votre API
         const response = await axios.get(`${apiUrl}/evenements/get_byId.php?id=${id}`);
         // console.log(response.data && response.data.recu && response.data.recu.length > 0)
         if (response.data && response.data.recu && response.data.recu.length > 0) {
            // Vérifiez que la réponse contient les données attendues
            console.log("la jointure",response.data.recu)
   ...
   const deletData = async (MyId) => {
      ...
      try {
         ...
         const response = await axios.post(`${apiUrl}/infos/delete_Info.php?id_evenement=${id}`, {
            headers: {
